...
use cosmwasm_std::{
    to_binary, Addr, DepsMut, Env, MessageInfo, Reply, Response, StdError, StdResult, WasmMsg,
};
...
pub fn instantiate(
    ...
    let cfg = Config {
        contract_owner: deps.api.addr_validate(&msg.cw721_minter)?,
        protocol_fee_bps: msg.protocol_fee_bps,
        cw721_contract_addr: Addr::unchecked(""),
    };
    CONFIG.save(deps.storage, &cfg)?;

    // TODO: converto into SubMsg
    ...
    Ok(Response::new()
        ...
        .add_message(instatiate_cw721_msg))
}
